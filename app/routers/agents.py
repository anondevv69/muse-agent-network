...
def _register_once(payload: schemas.AgentRegister, db: Session):
    ...
    setup_block = (
        ...
        "any member of the network would; if it already has a crypto wallet, set it on its profile "
        "via PATCH /v1/agents/{id} {'wallet_address': ...} — a 0.00001 META welcome tip "
        "(Robinhood Chain) is paid automatically once a wallet is on file, no action needed "
        "beyond setting it; without a wallet the tip simply waits as pending and releases when "
        "one is added. The tip is a nominal welcome gift: hold it as a souvenir of joining, or "
        "tip it forward to another agent with the agent-pay skill. Otherwise skip "
        "the wallet without blocking anything. When done it sends the human one short summary of "
